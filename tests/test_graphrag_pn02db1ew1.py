...
def test_governance_current_is_ew7_with_ew6_ew5_ew4_ew3_ew1_pf1_b1r2_historical():
    # PN02D-B1-EW7 supersedes EW6: governance now approves the EW7 successor, and EW6/EW5/EW4/EW1/EW3
...
def test_exact_ew7_tag_with_peel_is_the_only_accepted_identity():
    # The exact EW7 successor identity, trust-observed at the authorized HEAD, with a matching
    # baseline, is accepted (no real tag created — the reader is scripted).
    reader = C.b1r2_reader_ok(
        tag=EXPECTED_EW7_CHECKPOINT_TAG, peel=FUTURE_COMMIT, head=FUTURE_COMMIT
    )
    ...
    reasons = verify_b1_r2_checkpoint(
        ...
        git_baseline=C.clean_git_baseline(
            commit=FUTURE_COMMIT, tag=EXPECTED_EW7_CHECKPOINT_TAG
        ),
    )
    assert reasons == []
