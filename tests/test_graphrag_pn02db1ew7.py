...
deliberate waits without blocking. They also cover the EW7->EW8 governance repoint (current
approved checkpoint == EW8, EW7/EW6/EW5/... historical), lifecycle State A/B, and substitution
...
# --------------------------------------------------------------------------- #
# EW7 now HISTORICAL + EW8 successor governance (repointed by PN02D-B1-EW8)
# --------------------------------------------------------------------------- #


def test_ew7_now_historical_current_is_ew8():
    # PN02D-B1-EW8 repoint: the current approved provider-authorization identity is the exact
    # EW8 successor tag; the re-exported grant/manifest identity follows it. EW7 is now
    # HISTORICAL (its constant/tag preserved, no longer current), as EW6 already was.
    assert (
...
def test_ew7_historical_tag_git_state_is_lifecycle_valid():
    # PN02D-B1-EW8: EW7 is now HISTORICAL. Lifecycle-aware (never a permanent real-tag-absence
    # assumption). STATE A (fresh clone): the EW7 tag is absent → empty peel. STATE B (EW7 tag
    # present): it peels to a valid 40-hex commit — but as a HISTORICAL tag it need NOT equal the
    # current authorized HEAD (a future EW8 checkpoint moves HEAD past it).
    ...
    if not obs.observed_tag_exists:
        ...
        )  # STATE B — historical peel, NOT required to equal HEAD


def test_ew7_cannot_substitute_at_a_future_head():
    # PN02D-B1-EW8: once EW8 source moves HEAD, the (now historical) EW7 tag peeling to its OWN
    # commit ab90202 no longer peels to the authorized HEAD → cannot authorize (fail-closed).
    future = "e8e8e8e8" + "0" * 32
    ew7_commit = "ab902020" + "0" * 32
    grant = C.frozen_test_grant(b1_r2_checkpoint=EXPECTED_EW7_CHECKPOINT_TAG)
    reasons = verify_b1_r2_checkpoint(
        reader=C.b1r2_reader_ok(tag=EXPECTED_EW7_CHECKPOINT_TAG, peel=ew7_commit, head=future),
...
def test_ew8_exact_tag_with_peel_is_accepted_by_git_gate():
    # PN02D-B1-EW8: the EXACT EW8 successor identity, trust-observed at the authorized HEAD with a
    # matching baseline, satisfies the control-plane Git gate (no real tag created).
    future = "e8e8e8e8" + "0" * 32
...
def test_wrong_ew8_peel_fails_closed():
    # §39: the exact EW8 tag observed with a peel that does NOT match the authorized HEAD fails
    # closed.
    head = "aaaa1111" + "0" * 32
    wrong = "bbbb2222" + "0" * 32
    grant = C.frozen_test_grant(b1_r2_checkpoint=EXPECTED_EW8_CHECKPOINT_TAG)
...
def test_historical_or_arbitrary_tag_cannot_substitute_for_ew8(substitute):
    # §38: naming ANY historical (EW7/EW6/EW5/EW4/EW3/EW2/EW1/PF1/B1-R2) or arbitrary tag as the
    # approved-expected identity cannot satisfy the EW8 checkpoint.
    future = "e8e8e8e8" + "0" * 32
    grant = C.frozen_test_grant(b1_r2_checkpoint=EXPECTED_EW8_CHECKPOINT_TAG)
    reasons = verify_b1_r2_checkpoint(
...
def test_ew7_and_ew6_are_distinct_historical_identities():
    # §38: EW7/EW6 (and all earlier) remain DISTINCT retained-historical constants; EW8 current.
    ids = {
        EXPECTED_EW8_CHECKPOINT_TAG,
        EXPECTED_EW7_CHECKPOINT_TAG,
        EXPECTED_EW6_CHECKPOINT_TAG,
    ...
    assert len(ids) == 10  # all ten identities are distinct
    assert EXPECTED_EW7_CHECKPOINT_TAG == "graphrag-pn02db1ew7-index-observation-timing-approved"


def test_ew8_tag_alone_does_not_authorize_provider_run():
    # §39: even with the Git gate satisfiable (exact EW8 tag at HEAD), the provider-run
