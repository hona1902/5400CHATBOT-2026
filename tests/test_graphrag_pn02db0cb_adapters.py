...
def test_current_approved_b1_r2_checkpoint_is_expected_tag():
    # Governance (post PN02D-B1-EW2 §31/§32): the approved identity is now FROZEN to the exact
    # SUCCESSOR EW2 tag (which supersedes the historical EW1 checkpoint — the isolated-model-
    # seed fix moves HEAD past 1b8ca5b). It is non-None, but that tag does not exist in Git
    # yet, so the mint still fails closed (see the direct-mint tests). No caller override.
    from open_notebook.integrations.graphrag.eval.authmintlivepn02d import (
