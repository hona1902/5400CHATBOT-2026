...
def test_b3_live_identity_is_b3yr1_successor_and_historical_tags_separate():
    # PN02D-B3Y-R1: the governed B3 LIVE authorization identity is the predeclared B3Y-R1 successor tag,
    # DISTINCT from the historical B3B wiring tag AND the historical B3J/B3Q/B3V/B3X-R2/B3X-R4 successor tags
    # (all now ancestors / immutable evidence, NEVER the current live identity).
    assert EXPECTED_B3_LIVE_CHECKPOINT_TAG == "graphrag-pn02db3yr1-b3-live-auth-successor-approved"
    ...
    # the six historical successor/wiring tags are mutually distinct
    assert len({
        HISTORICAL_B3B_CHECKPOINT_TAG,
        HISTORICAL_B3J_CHECKPOINT_TAG,
        HISTORICAL_B3Q_CHECKPOINT_TAG,
    ...
    # resolver + backward-compat alias both point at the LIVE (B3Y-R1) identity, distinct from B2
...
def test_real_git_b3p_implementation_tag_is_ancestor_and_cannot_authorize_b3_live():
    ...
    # governed B3 live-auth identity (the B3Y-R1 successor tag, currently absent). The B3P tag peels to an
    ...
    # the grant's B3-live identity does not match the approved B3Y-R1 identity -> fail closed
...
def test_real_git_b3u_implementation_tag_is_ancestor_and_cannot_authorize_b3_live():
    ...
    # governed B3 live-auth identity (the B3Y-R1 successor tag, currently absent). The B3U tag peels to an
    ...
    # exact-HEAD impl tag is NOT the B3Y-R1 live identity -> fail closed on identity mismatch
    assert "b1_r2_grant_identity_mismatch" in reasons
...
def test_real_git_b3xr1_implementation_tag_is_ancestor_and_cannot_authorize_b3_live():
    ...
    # IMPLEMENTATION-evidence, NOT the governed B3 live-auth identity (the current B3Y-R1 successor tag).
    ...
    # ancestor implementation-evidence tag is NOT the B3Y-R1 live identity -> fail closed
    assert "b1_r2_grant_identity_mismatch" in reasons
...
def test_synthetic_exact_head_implementation_tag_cannot_authorize():
    ...
    # live-auth identity (B3Y-R1) is ABSENT, and prove FAIL_CLOSED through the SHARED production trust
    ...
    # identity. A scripted Git boundary models BOTH tags (impl present@HEAD / B3Y-R1 absent), so the
    ...
    expected_live = EXPECTED_B3B_CHECKPOINT_TAG  # the current B3Y-R1 successor identity
    ...
    def _multi_tag_runner(args):
        ...
        if a[:2] == ["tag", "--list"]:
            ...
            # the implementation-evidence tag exists; the expected live (B3Y-R1) tag is ABSENT
    ...
    # ... while the expected live identity (B3Y-R1) is ABSENT
    live_obs = reader.observe(expected_live)
    assert live_obs.observed_tag_exists is False
    # a grant naming the (correct) expected live identity B3Y-R1 fails closed because that tag is ABSENT,
