...
the completed EW4→EW5 governance repoint (current approved checkpoint == EW5, EW4 historical),
and the EW5 successor-checkpoint lifecycle (State A/B). No provider/model change, no
...
# --------------------------------------------------------------------------- #
# §40/§41/§42 — EW5 successor checkpoint lifecycle + completed governance repoint
# --------------------------------------------------------------------------- #

def test_ew5_governance_repoint_complete():
    # Remediation #1 (M2): the EW4→EW5 governance repoint is COMPLETE in-repo — the current
    # approved provider-authorization identity is the exact EW5 successor tag, and the
    # re-exported grant/manifest identity follows it. EW4 is now HISTORICAL (not current).
    # The checkpoint turn therefore needs NO further governance code change.
    assert (
        EXPECTED_EW5_CHECKPOINT_TAG
...
def test_ew5_successor_tag_git_state_is_lifecycle_valid():
    # §42: lifecycle-aware — never a permanent real-tag-absence assumption. STATE A
    # (pre-checkpoint): the EW5 tag is absent -> empty peel. STATE B (post-checkpoint): the
    # EXACT tag is present, peels to a 40-hex commit == the authorized HEAD.
...
def test_ew5_exact_tag_with_peel_is_accepted_by_git_gate():
    # §32/§39: the EXACT EW5 successor identity, trust-observed at the authorized HEAD with a
...
def test_wrong_ew5_peel_fails_closed():
    # §32: the exact EW5 tag observed with a peel that does NOT match the authorized HEAD fails
    # closed (a tag pointing somewhere other than the approved HEAD can never authorize).
    head = "aaaa1111" + "0" * 32
    wrong = "bbbb2222" + "0" * 32
    grant = C.frozen_test_grant(b1_r2_checkpoint=EXPECTED_EW5_CHECKPOINT_TAG)
    reasons = verify_b1_r2_checkpoint(
        reader=C.b1r2_reader_ok(tag=EXPECTED_EW5_CHECKPOINT_TAG, peel=wrong, head=head),
        operator_grant=grant,
        approved_expected_checkpoint=EXPECTED_EW5_CHECKPOINT_TAG,
        git_baseline=C.clean_git_baseline(commit=head, tag=EXPECTED_EW5_CHECKPOINT_TAG),
    )
    assert "b1_r2_tag_not_at_authorized_head" in reasons
...
def test_historical_or_arbitrary_tag_cannot_substitute_for_ew5(substitute):
    # §31/§35: naming ANY historical (EW4/EW3/EW2/EW1/PF1/B1-R2) or arbitrary tag as the
    # approved-expected identity cannot satisfy the EW5 checkpoint — the grant's B1-R2 identity
    # (the current EW5 successor) must EXACTLY equal the governance-approved identity
    ...
    assert reasons, f"{substitute} must not satisfy the EW5 checkpoint"


def test_ew5_tag_alone_does_not_authorize_provider_run():
    # §33/§36: even with the Git gate satisfiable (exact EW5 tag at HEAD), the provider-run
