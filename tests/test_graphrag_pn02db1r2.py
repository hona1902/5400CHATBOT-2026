...
def test_expected_b1_r2_tag_frozen_in_governance():
    # Governance now approves the SUCCESSOR EW7 checkpoint (the bounded index-observation-timing
    ...
    assert EXPECTED_EW7_CHECKPOINT_TAG != EXPECTED_EW4_CHECKPOINT_TAG
...
def test_current_approved_checkpoint_git_state_is_valid():
    # CHECKPOINT-LIFECYCLE aware for the CURRENT approved identity (EW7). State A = the
    # successor tag is absent (pre-checkpoint) → gate unsatisfied; State B = the exact EW7
    # tag exists at the authorized HEAD. Stays green before AND after the EW7 checkpoint,
    ...
    assert approved == EXPECTED_EW7_CHECKPOINT_TAG
...
def test_manifest_frozen_and_content_safe():
    ...
    assert m["expected_b1_r2_checkpoint_tag"] == EXPECTED_EW7_CHECKPOINT_TAG
...
def test_git_gate_satisfiable_does_not_authorize_provider_run():
    ...
    reader = C.b1r2_reader_ok(
        tag=EXPECTED_EW7_CHECKPOINT_TAG, peel=FUTURE_B1_R2_COMMIT, head=FUTURE_B1_R2_COMMIT
    )
    assert verify_b1_r2_checkpoint(
        ...
        git_baseline=C.clean_git_baseline(commit=FUTURE_B1_R2_COMMIT, tag=EXPECTED_EW7_CHECKPOINT_TAG),
...
def test_future_positive_full_mint_via_production_signature_under_patch():
    ...
    baseline = C.clean_git_baseline(commit=FUTURE_B1_R2_COMMIT, tag=EXPECTED_EW7_CHECKPOINT_TAG)
    preflight = mint_real_preflight_authorization(
        gate0_passed=True, gate1_passed=True, fixture_hash=_fixture_hash(),
        run_id=B.B1_RUN_ID, runtime_count=3,
    )
    with C.approved_b1r2_governance(tag=EXPECTED_EW7_CHECKPOINT_TAG, head=FUTURE_B1_R2_COMMIT):
    ...
    assert auth.b1_r2_checkpoint == EXPECTED_EW7_CHECKPOINT_TAG
