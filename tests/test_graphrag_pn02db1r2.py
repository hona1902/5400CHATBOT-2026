...
def test_expected_b1_r2_tag_frozen_in_governance():
    # Governance now approves the SUCCESSOR EW4 checkpoint (the Boot-2 execution
    # readiness-parity fix moves HEAD past the historical EW3 commit 9f41728, superseding the
    # EW3 Git gate). EW3/EW1/PF1/B1-R2/EW2 remain DISTINCT, retained historical constants.
...
def test_current_approved_checkpoint_git_state_is_valid():
    # CHECKPOINT-LIFECYCLE aware for the CURRENT approved identity (EW4). State A = the
    # successor tag is absent (pre-checkpoint) → gate unsatisfied; State B = the exact EW4
    # tag exists at the authorized HEAD. Stays green before AND after the EW4 checkpoint,
    # and is UNAFFECTED by the historical EW3/EW1/PF1/B1-R2/EW2 tags (see the immutability test below).
...
def test_manifest_frozen_and_content_safe():
    ...
    assert m["expected_b1_r2_checkpoint_tag"] == EXPECTED_EW4_CHECKPOINT_TAG
    ...
    # present) → True. This stays truthful across the EW4 checkpoint lifecycle instead of
...
def test_git_gate_satisfiable_does_not_authorize_provider_run():
    ...
    reader = C.b1r2_reader_ok(
        tag=EXPECTED_EW4_CHECKPOINT_TAG, peel=FUTURE_B1_R2_COMMIT, head=FUTURE_B1_R2_COMMIT
    )
    assert verify_b1_r2_checkpoint(
        ...
        git_baseline=C.clean_git_baseline(commit=FUTURE_B1_R2_COMMIT, tag=EXPECTED_EW4_CHECKPOINT_TAG),
...
def test_future_positive_full_mint_via_production_signature_under_patch():
    ...
    baseline = C.clean_git_baseline(commit=FUTURE_B1_R2_COMMIT, tag=EXPECTED_EW4_CHECKPOINT_TAG)
    preflight = mint_real_preflight_authorization(
        gate0_passed=True, gate1_passed=True, fixture_hash=_fixture_hash(),
        run_id=B.B1_RUN_ID, runtime_count=3,
    )
    with C.approved_b1r2_governance(tag=EXPECTED_EW4_CHECKPOINT_TAG, head=FUTURE_B1_R2_COMMIT):
    ...
    assert auth.b1_r2_checkpoint == EXPECTED_EW4_CHECKPOINT_TAG
