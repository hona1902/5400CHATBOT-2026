...
# the PN02D-B1-EW3 isolation-runtime-id fix superseded it, and after the EW3 -> EW4 -> EW5 -> EW6 -> EW7
# supersession chain the CURRENT approved identity is EW7. The current-successor governance +
# checkpoint-lifecycle (State-A/State-B) coverage lives in tests/test_graphrag_pn02db1ew7.py.
# Test-only, zero provider traffic.
# --------------------------------------------------------------------------- #

def test_governance_ew2_is_historical_current_is_ew7():
    ...
    # EW7 is now the frozen approved provider-authorization identity (EW6 superseded); EW2
...
def test_ew2_historical_tag_immutable_and_cannot_substitute_for_ew7():
    ...
    # (that is EW7), and naming it as the approved-expected identity for a later/successor
