...
is NOT currently mintable (the current governance-approved checkpoint tag — the PN02D-B1-EW6
index-conflict-recovery tag — does not yet exist).
...
Temporal correctness (task §6/§7/§8; PN02D-B1-EW1 §38/§39). THREE DISTINCT checkpoints:
  ...
  * HISTORICAL — ``authmintlivepn02d.EXPECTED_EW3_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew3-isolation-id-compat-approved``): peels to ``9f41728`` and WAS the
    governance-approved identity through EW3. The EW4 Boot-2 execution readiness-parity fix
    moves HEAD past ``9f41728``, so it no longer peels to the authorized HEAD and is NO LONGER
    approved.
  * HISTORICAL — ``authmintlivepn02d.EXPECTED_EW4_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew4-boot2-readiness-approved``): peels to ``1949b92`` and WAS the
    governance-approved identity through EW4. The EW5 sanitized provider-error-observability
    change moves HEAD past ``1949b92``, so it no longer peels to the authorized HEAD and is
    NO LONGER approved.
  * HISTORICAL — ``authmintlivepn02d.EXPECTED_EW5_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew5-provider-error-observability-approved``): peels to ``8b0325a`` and
    WAS the governance-approved identity through EW5. The EW6 non-destructive index-conflict
    recovery change moves HEAD past ``8b0325a``, so it no longer peels to the authorized HEAD
    and is NO LONGER approved.
  * CURRENT — ``authmintlivepn02d.EXPECTED_EW6_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew6-index-conflict-recovery-approved``): the SUCCESSOR that
...
such tag, and the live mint FAILS CLOSED. Only a FUTURE operator-approved EW6 checkpoint
that creates the exact annotated tag (peeling to the approved EW6 HEAD) can make the Git
...
#: truth is ``authmintlivepn02d.EXPECTED_EW6_CHECKPOINT_TAG`` (the PN02D-B1-EW6 successor
#: that supersedes the historical EW5 checkpoint — the non-destructive index-conflict-recovery
#: change moves HEAD past ``8b0325a``); re-exported here for the grant/manifest. The annotated
#: tag does not exist yet, so the live mint stays FAIL CLOSED.
B1_R2_EXPECTED_CHECKPOINT_TAG = EXPECTED_EW6_CHECKPOINT_TAG
...
def b1_r2_authorization_manifest(
    *,
    ...
    present → ``True``), so it stays truthful across the EW6 checkpoint lifecycle instead of
...
def b1_r2_preflight(
    *,
    ...
    Observes real Git for the current EXPECTED checkpoint tag (the PN02D-B1-EW6
