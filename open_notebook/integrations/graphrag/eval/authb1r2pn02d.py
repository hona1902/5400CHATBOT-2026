...
is NOT currently mintable (the current governance-approved checkpoint tag — the PN02D-B1-EW5
...
Temporal correctness (task §6/§7/§8; PN02D-B1-EW1 §38/§39). THREE DISTINCT checkpoints:
  ...
  * HISTORICAL — ``authmintlivepn02d.EXPECTED_EW4_CHECKPOINT_TAG``
    ...
    NO LONGER approved.
  * CURRENT — ``authmintlivepn02d.EXPECTED_EW5_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew5-provider-error-observability-approved``): the SUCCESSOR that
...
such tag, and the live mint FAILS CLOSED. Only a FUTURE operator-approved EW5 checkpoint
that creates the exact annotated tag (peeling to the approved EW5 HEAD) can make the Git
...
#: truth is ``authmintlivepn02d.EXPECTED_EW5_CHECKPOINT_TAG`` (the PN02D-B1-EW5 successor
#: that supersedes the historical EW4 checkpoint — the sanitized provider-error-observability
#: change moves HEAD past ``1949b92``); re-exported here for the grant/manifest. The annotated
#: tag does not exist yet, so the live mint stays FAIL CLOSED.
B1_R2_EXPECTED_CHECKPOINT_TAG = EXPECTED_EW5_CHECKPOINT_TAG
...
def b1_r2_authorization_manifest(
    *,
    ...
    present → ``True``), so it stays truthful across the EW5 checkpoint lifecycle instead of
...
def b1_r2_preflight(
    *,
    ...
    Observes real Git for the current EXPECTED checkpoint tag (the PN02D-B1-EW5 successor
