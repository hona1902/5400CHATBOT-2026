...
is NOT currently mintable (the current governance-approved checkpoint tag — the PN02D-B1-EW4
...
Temporal correctness (task §6/§7/§8; PN02D-B1-EW1 §38/§39). THREE DISTINCT checkpoints:
  ...
  * HISTORICAL — ``authmintlivepn02d.EXPECTED_EW3_CHECKPOINT_TAG``
    ...
    approved.
  * CURRENT — ``authmintlivepn02d.EXPECTED_EW4_CHECKPOINT_TAG``
    (``graphrag-pn02db1ew4-boot2-readiness-approved``): the SUCCESSOR that governance now
...
such tag, and the live mint FAILS CLOSED. Only a FUTURE operator-approved EW4 checkpoint
that creates the exact annotated tag (peeling to the approved EW4 HEAD) can make the Git
...
#: truth is ``authmintlivepn02d.EXPECTED_EW4_CHECKPOINT_TAG`` (the PN02D-B1-EW4 successor
#: that supersedes the historical EW3 checkpoint — the Boot-2 execution readiness-parity fix
#: moves HEAD past ``9f41728``); re-exported here for the grant/manifest. The annotated tag
#: does not exist yet, so the live mint stays FAIL CLOSED.
B1_R2_EXPECTED_CHECKPOINT_TAG = EXPECTED_EW4_CHECKPOINT_TAG
...
def b1_r2_preflight(
    *,
    ...
    Observes real Git for the current EXPECTED checkpoint tag (the PN02D-B1-EW4 successor
