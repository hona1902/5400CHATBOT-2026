...
#: HISTORICAL evidence of the B3V live-auth successor checkpoint. IMMUTABLE (tag object 38aa7a01,
#: peel 6fe88fa); it peeled to HEAD while B3V was the current live identity. The approved PN02D-B3X-R1
#: QA-value runtime-wiring implementation checkpoint then advanced HEAD to a577274, so this tag is now
#: an ANCESTOR and is retained ONLY as historical provenance — it is NEVER the current governed B3
#: LIVE authorization identity (using it now fails closed under exact-HEAD trust, the stale-successor
#: condition, no grandfathering — as PN02D-B3X-R1's post-checkpoint probe confirmed).
HISTORICAL_B3V_CHECKPOINT_TAG = "graphrag-pn02db3v-b3-live-auth-successor-approved"

#: PN02D-B3X-R2: the PREDECLARED successor trust anchor for a governed real B3 LIVE authorization at the
#: CURRENT (B3X-R1-checkpointed) HEAD. This annotated tag does NOT exist yet — it is created ONLY by a
#: future, separately-authorized B3X-R2 checkpoint (after independent review) pointing at that new commit,
#: so its peel then equals the new HEAD and exact-HEAD trust is satisfiable WITHOUT advancing HEAD past
#: its own identity (the recurring drift the B3B→B3J→B3Q→B3V→B3X-R2 chain fixes; the B3X-R1/B3U/B3P
#: implementation tags must NOT be reused as the live identity — B3X-R1 peels to the current HEAD but is
#: QA-value runtime-wiring implementation EVIDENCE, not a live-auth identity, so it must be rejected).
#: While this tag is ABSENT from real Git the B3 live mint FAILS CLOSED (tag-not-observed) — the correct,
#: intended state during implementation/review. Only the tag NAME is depended on here; no future
#: commit/tag object SHA is hardcoded. Predecessors (all IMMUTABLE, now ancestors, NEVER the live
#: identity): ``HISTORICAL_B3B_CHECKPOINT_TAG``, ``HISTORICAL_B3J_CHECKPOINT_TAG``,
#: ``HISTORICAL_B3Q_CHECKPOINT_TAG``, ``HISTORICAL_B3V_CHECKPOINT_TAG``.
EXPECTED_B3_LIVE_CHECKPOINT_TAG = "graphrag-pn02db3xr2-b3-live-auth-successor-approved"

#: Backward-compatible alias. The governed B3 LIVE authorization identity is now the B3X-R2 successor
...
#: / ``HISTORICAL_B3V_CHECKPOINT_TAG``. This alias is retained so existing importers/tests that refer to
...
#: authorized HEAD; else fails closed. B1/EW8, the B2 tag, the historical B3B/B3J/B3Q/B3V tags (now
#: ancestors), and the B3P/B3U/B3X-R1 implementation tags can never substitute — exact-identity +
...
def current_approved_b3b_checkpoint() -> Optional[str]:
    ...
    OBSERVABILITY run (PN02D-B3D/B3J/B3Q/B3V/B3X-R2). Returns the predeclared successor
    ``EXPECTED_B3_LIVE_CHECKPOINT_TAG`` (``graphrag-pn02db3xr2-b3-live-auth-successor-approved``) — a
    ...
    (``HISTORICAL_B3B_CHECKPOINT_TAG``), the historical B3J/B3Q/B3V successor tags
    (``HISTORICAL_B3J_CHECKPOINT_TAG`` / ``HISTORICAL_B3Q_CHECKPOINT_TAG`` / ``HISTORICAL_B3V_CHECKPOINT_TAG``)
    — all now ancestors — the B3H governance tag, and the B3P/B3U/B3X-R1 implementation tags. Non-None, but a B3 live authorization is
...
def b3b_r2_refusal_reasons(operator_grant: object, git_baseline: object) -> List[str]:
    ...
    LIVE identity, i.e. the ``EXPECTED_B3_LIVE_CHECKPOINT_TAG`` B3X-R2 successor tag — NOT the
...
#: LIVE identity (``EXPECTED_B3_LIVE_CHECKPOINT_TAG``, the B3X-R2 successor tag) — it FAILS CLOSED
...
def mint_live_b3_provider_run_authorization(
    *,
    ...
    ``EXPECTED_B3_LIVE_CHECKPOINT_TAG``, the B3X-R2 successor tag) plus the B2-style caps
...
def frozen_b3b_operator_grant_template(
    *,
    ...
    i.e. the ``EXPECTED_B3_LIVE_CHECKPOINT_TAG`` B3X-R2 successor tag — not the historical B3B tag),
