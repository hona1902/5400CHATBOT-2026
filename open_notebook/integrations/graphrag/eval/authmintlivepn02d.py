...
# identity comes from GOVERNANCE (``current_approved_b1_r2_checkpoint``), now the PN02D-B1-EW6
...
#: The configured approved identity is the EW6 checkpoint tag; these sentinels are never it, so
...
#: approved identity (that is now ``EXPECTED_EW6_CHECKPOINT_TAG``).
...
#: identity (that is now ``EXPECTED_EW6_CHECKPOINT_TAG``).
...
#: it is NO LONGER the governance-approved identity (that is now ``EXPECTED_EW6_CHECKPOINT_TAG``).
...
#: FROZEN to the PN02D-B1-EW6 SUCCESSOR checkpoint (the non-destructive index-conflict-recovery
#: change that moves HEAD past the historical EW5 commit ``8b0325a``). This is a control-plane
...
#: the live mint FAILS CLOSED (PN02D-B1-EW6 §33). Only after a future operator-approved EW6
#: checkpoint creates this exact annotated tag (peeling to the approved EW6 HEAD) can the mint
#: become satisfiable. Freezing the EXPECTED identity before the tag exists removes circularity
#: (the same pattern used for B1-R2 → PF1 → EW1 → EW2 → EW3 → EW4 → EW5).
...
#: ``EXPECTED_EW6_CHECKPOINT_TAG`` (SOLE source of the approved-EXPECTED identity for the
...
#: ``b1_r2_tag_not_observed_in_git`` (PN02D-B1-EW6 §33). The mint remains FAIL CLOSED before
#: the successor tag exists. EW1, PF1, B1-R2, EW2, EW3, EW4 and EW5 are retained as HISTORICAL
#: identities only and can never substitute (exact-identity + trusted-reader binding — §35).
_APPROVED_B1_R2_CHECKPOINT: Optional[str] = EXPECTED_EW6_CHECKPOINT_TAG
...
def current_approved_b1_r2_checkpoint() -> Optional[str]:
    ...
    Returns the frozen ``EXPECTED_EW6_CHECKPOINT_TAG`` — the PN02D-B1-EW6 SUCCESSOR that
    supersedes the historical EW5 checkpoint (the non-destructive index-conflict-recovery change
    moves HEAD past ``8b0325a``, so the EW5 tag no longer peels to the authorized HEAD). It is
    ...
    with ``b1_r2_tag_not_observed_in_git`` (PN02D-B1-EW6 §33). The value is NEVER derived from
    a caller string, a Git-tag naming heuristic, or a prefix match. EW1, PF1, B1-R2, EW2, EW3,
    EW4 and EW5 remain HISTORICAL identities that can never substitute for the current EW6 successor.
...
class RealTrustedB1R2Reader:
    ...
    unit-testable without creating a real tag). The current approved EW6 checkpoint tag does
...
def b1_r2_refusal_reasons(operator_grant: object, git_baseline: object) -> List[str]:
    ...
    EW6 checkpoint tag; while that annotated tag is ABSENT from real Git it fails closed. It
...
def mint_live_provider_run_authorization(
    *,
    ...
    Governance returns the EW6 checkpoint tag; while that annotated tag is ABSENT from real Git
    ...
    #     no parameter to substitute either. Governance returns the EW6 checkpoint tag; while
